...
export function PrimaryButton({ label, onPress, icon = 'arrow.right' }: { label: string; onPress: () => void; icon?: any }) {
  return <Pressable onPress={onPress} style={({ pressed }) => [{ transform: [{ scale: pressed ? 0.97 : 1 }], flexDirection: 'row', alignItems: 'center', justifyContent: 'center', borderRadius: 16, backgroundColor: '#2D5A27', paddingHorizontal: 20, paddingVertical: 16 }]}><Text className="font-body text-sm font-extrabold text-white">{label}</Text><IconSymbol name={icon} size={17} color="#FFFFFF" style={{ marginLeft: 8 }} /></Pressable>;
}
